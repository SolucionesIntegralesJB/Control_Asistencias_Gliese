...
function toggleFechaVencimiento() {
    var voucherType = $("#create_income_details_form :input[name=vt_description]").val();
    var paymentMethod = $("#create_income_details_form :input[name=fp_description]").val();

    if (voucherType === "1" && paymentMethod === "2") { 

        $("#create_income_details_form :input[name=fecha_vencimiento]").prop("readonly", false);
    } else if (voucherType === "2" || (voucherType === "1" && paymentMethod === "2")) { 
