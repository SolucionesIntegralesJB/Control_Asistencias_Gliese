...
function destroy_datatable() {
    ...
    let dataTable = $("#datatable-billingpersale").DataTable({
      ...
      columns: [
        ...
        },
        {
          ...
          render: function (data, type, row, meta) {
            return (
              '<button class="btn btn-sm btn-light btn-icon btn_sunat" data-process-key="' +
              row.id_billingpersale +
              '">' +
              '<img src="' +
              BASE_URL +
              'public/app-assets/images/logo/sunat.svg" style="width: 25px; height: 25px;" alt="SUNAT">' +
              "</button>" +
              " " +
              '<button class="btn btn-sm btn-light btn-round btn-icon btn_pdf" data-process-key="' +
              row.id_billingpersale +
              '_1" target="_blank">' +
              '<img src="' +
              BASE_URL +
              'public/app-assets/images/svg/pdf.svg" style="width: 25px; height: 25px;" alt="File Text">' +
              "</button>" +
              " " +
              '<button class="btn btn-sm btn-light btn-round btn-icon btn_pdf" data-process-key="' +
              row.id_billingpersale +
              ...
              '<img src="' +
              BASE_URL +
              'public/app-assets/images/svg/receipt.svg" style="width: 25px; height: 25px;" alt="File Text">' +
              "</button>" +
              " " +
              '<button class="btn btn-sm btn-light btn-round btn-icon btn_xml" data-process-key="' +
              row.id_billingpersale +
              '_2" target="_blank">' +
              '<img src="' +
              BASE_URL +
              'public/app-assets/images/svg/xml.svg" style="width: 25px; height: 25px;" alt="File Text">' +
              "</button>" +
              " " +
              '<button class="btn btn-sm btn-light btn-round btn-icon btn_send" data-process-key="' +
              row.id_billingpersale +
              '">' +
              '<img src="' +
              BASE_URL +
              'public/app-assets/images/svg/send.svg" style="width: 25px; height: 25px;" alt="Enviar">' +
              "</button>"
            );
          },
        },
      ...
      buttons: [
        ...
        ...functions.custom_buttons_datatable(
          [7],
  ...
  function showWarningAlert(warning) {
    Swal.mixin({
      ...
      .fire({
        icon: "warning",
        title:
          '<i class="fas fa-exclamation-triangle"></i> ¡Documentos Pendientes!',
        html: `
  ...
  function clearFilters() {
    ...
    functions.toast_message("info", "Filtros eliminados", "OK");
  }
  ...
  $(document).on("click", ".btn_pdf", function () {
    ...
    let url =
      BASE_URL +
      "Billingpersale/get_billingpersale_Report?id_billingpersale=" +
      id_billingpersale +
      "&tipo=" +
      tipo;
    console.log(url);
  ...
  $(document).on("click", ".btn_xml", function () {
    let value = $(this).attr("data-process-key");
    let url =
      BASE_URL + "Billingpersale/xml_billingpersale?id_billingpersale=" + value;
  ...
  $(document).on("click", ".btn_sunat", function () {
    let value = $(this).attr("data-process-key");
    let url =
      BASE_URL + "Billingpersale/Emitir_comprobante?id_billingpersale=" + value;
    Swal.fire({
      title: "Procesando...",
      ...
      success: function (response) {
        if (response.success) {
          ...
          if (response.observaciones && response.observaciones.length > 0) {
            mensaje += `<strong>Observaciones:</strong><br>${response.observaciones.join(
              "<br>"
            )}`;
          } else {
  ...
  $("#send_modal").on("hidden.bs.modal", function () {
    if (
      $("#send_form").length > 0 &&
      typeof $("#send_form")[0].reset === "function"
    ) {
      $("#send_form")[0].reset();
    } else {
      $("#emailInput").val("");
    ...
    $.ajax({
      ...
      success: function (data) {
        ...
        if (data.status === "OK") {
          ...
          let item = data.data;
          // --
          $("#update_proforma_form :input[name=id_proforma]").val(
            item.id_proforma
          );
          $("#update_proforma_form :input[name=id_category]").val(
            item.id_category
          );
          $("#update_proforma_form :input[name=description]").val(
            item.description
          );
  ...
  document.addEventListener("DOMContentLoaded", function () {
    ...
    iconos.forEach((icono) => {
      ...
      if (elemento) {
        cargarIconoSvg(
          elemento,
          BASE_URL + "public/app-assets/images/svg/" + icono.ruta
        );
      }
    });
  });
  ...
  function send_whatsapp() {
    ...
    if (/^9\d{8}$/.test(whatsappNumber)) {
      ...
      alert(
        "Por favor, ingrese un número de WhatsApp válido (9 dígitos comenzando con 9)"
      );
    }
  }
