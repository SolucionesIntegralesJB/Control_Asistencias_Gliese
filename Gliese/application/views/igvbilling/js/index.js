...
function destroy_datatable() {
    // --
    $('#datatable-igvbilling').dataTable().fnDestroy();
}

// --
function refresh_datatable() {
    // --
    $('#datatable-igvbilling').DataTable().ajax.reload();
}
...
function load_datatable() {
    ...
    let dataTable = $('#datatable-igvbilling').DataTable({
            ...
            },
            {
                ...
                render: function (data, type, row, meta) {
                    return (
                        ...
                        '<button class="btn btn-sm btn-light btn-round btn-icon btn_pdf" data-process-key="' + row.id_igvbilling + '_1" target="_blank">' +
                        '<img src="' + BASE_URL + 'public/app-assets/images/svg/pdf.svg" style="width: 25px; height: 25px;" alt="File Text">' +
                        '</button>' +
                        ' ' +
                        '<button class="btn btn-sm btn-light btn-round btn-icon btn_pdf" data-process-key="' + row.id_igvbilling + '_2" target="_blank">' +
                        '<img src="' + BASE_URL + 'public/app-assets/images/svg/receipt.svg" style="width: 25px; height: 25px;" alt="File Text">' +
                        '</button>' +
                        ' ' +
                        
                        '<button class="btn btn-sm btn-light btn-round btn-icon btn_send" data-process-key="' + row.id_igvbilling + '">' +
        ...
        buttons: [
            ...
            ...functions.custom_buttons_datatable([7], '#create_igvbilling_modal')
        ],
        language: {
    ...
    $('#datatable-igvbilling').on('draw.dt', function () {
...
$(document).on('click', '.btn_pdf', function () {
    ...
    let [id_igvbilling, tipo] = value.split('_');
    // --
    let url = BASE_URL + 'Igvbilling/get_igvbilling_Report?id_igvbilling=' + id_igvbilling + '&tipo=' + tipo;
    console.log(url);
...
$(document).on('click', '.btn_xml', function () {
    let value = $(this).attr('data-process-key');
    let url = BASE_URL + 'Igvbilling/xml_igvbilling?id_igvbilling=' + value;
...
$(document).on('click', '.btn_sunat', function () {
    let value = $(this).attr('data-process-key');
    let url = BASE_URL + 'Igvbilling/Emitir_comprobante?id_igvbilling=' + value;
    Swal.fire({
        title: 'Procesando...',
...
$(document).on('click', '.btn_send', function () {
    let id_igvbilling = $(this).attr('data-process-key');
    $('#send_modal').data('id_igvbilling', id_igvbilling);
    $('#emailInput').val('').attr('placeholder', 'Cargando...');
    $('#whatsappInput').val('').attr('placeholder', 'Cargando...');
    $('#send_modal').modal('show');

    $.ajax({
        url: BASE_URL + 'Igvbilling/get_number_email',
        type: 'GET',
        data: { id_igvbilling: id_igvbilling },
        dataType: 'json',
        success: function (response) {
...
$(document).on('click', '.btn_update', function () {
    ...
    let value = $(this).attr('data-process-key');
    // --
    let params = { 'id_igvbilling': value }
    // --
    $.ajax({
        url: BASE_URL + 'Igvbilling/get_igvbilling_by_id',
        type: 'GET',
        data: params,
        dataType: 'json',
        contentType: false,
    ...
    // --
    $('#update_igvbilling_modal').modal('show');
})
...
$(document).on('click', '.btn_delete', function () {
    ...
    Swal.fire({
        ...
        preConfirm: _ => {
            return $.ajax({
                url: BASE_URL + 'Igvbilling/delete_igvbilling',
                type: 'POST',
                data: params,
                dataType: 'json',
                cache: false,
...
$(document).on('click', '.create-new', function () {
    // --
    window.location.assign(BASE_URL + 'Igvbilling_Details');
})


// -- Validate form
$('#update_igvbilling_form').validate({
    // --
    submitHandler: function (form) {
        update_igvbilling(form);
    }
})
...
function send_email(method) {
    let id_igvbilling = $('#send_modal').data('id_igvbilling');
    if (method === 'email') {
        ...
        $.ajax({
            url: BASE_URL + 'igvbilling/send_email',
            type: 'GET',
            data: {
                id_igvbilling: id_igvbilling,
                email: email
            },
            dataType: 'json',
            success: function (response) {
