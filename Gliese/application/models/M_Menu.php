...
class M_Menu extends Model {
    ...
    // --
    // Get menu for user_roles (practicantes/supervisores)
    public function get_menu_by_user_role($bind) {
        // --
        try {
            // --
            $sql = 'SELECT DISTINCT
                ...
                LEFT JOIN user_roles ur ON ur.id = p.id_user_role
                LEFT JOIN sub_menu sm ON sm.id = p.id_sub_menu
                LEFT JOIN menu m ON m.id = sm.id_menu
                WHERE p.status = 1 AND p.id_user_role = :id_user_role
                ORDER BY m.order, sm.order ASC';
            // --
            $result = $this->pdo->fetchAll($sql, $bind);
            // --
    ...
    // -- Obtener menús disponibles para user_roles (solo módulo Perfil)
    public function get_menu_for_user_roles() {
        try {
            $sql = 'SELECT
                    m.id as id_menu,
                    m.description as description_menu,
                ...
                FROM sub_menu sm
                INNER JOIN menu m ON m.id = sm.id_menu
                WHERE UPPER(TRIM(m.description)) = "PERFIL"
                ORDER BY m.order, sm.order ASC';
