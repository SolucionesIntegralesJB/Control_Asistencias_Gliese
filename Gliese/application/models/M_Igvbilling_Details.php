...
// --
class M_Igvbilling_Details extends Model
{
  ...
  // --
  public function create_igvbilling($billingData, $products)
  {
    try {
      $this->pdo->beginTransaction();

      $this->setSeriesAndCorrelative($billingData);
      $billingId = $this->insertIgvbilling($billingData);
      $this->insertIgvbillingDetails($billingId, $products, $billingData['fecha_emision']);
  ...
  {
    $sqlCheck = "SELECT MAX(series) as max_series, MAX(correlative) as max_correlative FROM `igvbilling` WHERE voucher_type = :vt_description";
  ...
  private function insertIgvbilling($billingData)
  {
    $sql = 'INSERT INTO igvbilling (operation_type,person_id, voucher_type, series, correlative, issue_date, issue_time, due_date, currency, payment_method, payment_medium, taxable_operations, free_operations, exempt_operations, unaffected_operations, igv, total_amount,leyend, user_id, campus_id, status) 
    ...
    $stmt = $this->pdo->prepare($sql);
    $stmt->execute($this->prepareIgvbillingData($billingData));
    return $this->pdo->lastInsertId();
  }

  private function prepareIgvbillingData($billingData)
  {
  ...
  private function insertIgvbillingDetails($billingId, $products, $saleDate)
  {
    $sqlDetail = 'INSERT INTO igvbilling_detail (sale_id, product_id, quantity, item, unit_type, code, description,serie, tax_percentage, tax_amount, tax_affectation_type, unit_value, free_unit_value, item_unit_price, sale_date, Type_taxation)
  ...
  {
    $igvbilling = $this->getIgvbilling($billingId);
    $igvbilling_details = $this->getIgvbillingDetails($billingId);
    $companyInfo = $this->getCompanyInfo();

    if ($igvbilling['code'] == 01) {
      $this->createCAB($igvbilling, $companyInfo);
      $this->createDET($igvbilling, $igvbilling_details, $companyInfo);
      $this->createLEY($igvbilling, $companyInfo);
      $this->createTRI($igvbilling, $companyInfo);
      $this->createPAG($igvbilling, $companyInfo);
    } elseif ($igvbilling['code'] == 03) {
      $this->createCAB($igvbilling, $companyInfo);
      $this->createDET($igvbilling, $igvbilling_details, $companyInfo);
      $this->createLEY($igvbilling, $companyInfo);
      $this->createTRI($igvbilling, $companyInfo);
  ...
  private function getIgvbilling($billingId)
  {
    $sqligvbilling = 'SELECT 
        b.id, 
    ...
    $stmt = $this->pdo->prepare($sqligvbilling);
    $stmt->bindParam(':id_sale', $billingId, PDO::PARAM_INT);
    $stmt->execute();
    return $stmt->fetch(PDO::FETCH_ASSOC);
  }

  private function getIgvbillingDetails($billingId)
  {
    $sqligvbilling_detail = 'SELECT 
        bd.sale_id,
        bd.product_id,
        bd.item,
        p.code, 		
    ...
    $stmt = $this->pdo->prepare($sqligvbilling_detail);
    $stmt->bindParam(':id_sale', $billingId, PDO::PARAM_INT);
    $stmt->execute();
    return $stmt->fetchAll(PDO::FETCH_ASSOC);
  }

  private function createCAB($igvbilling, $companyInfo)
  {
    $ublVersionId = "2.1";
    $customizationId = "2.0";
    $codLocalEmisor = "0000";
    $codDoc = ($igvbilling['document_type'] == "RUC") ? "6" : "1";
    $sumDescTotal = "0.00";
    $sumOtrosCargos = "0.00";
    $sumTotalAnticipos = "0.00";

    $cabecera = [
      $igvbilling['operation_type'],
      $igvbilling['date'],
      $igvbilling['issue_time'],
      $igvbilling['due_date'],
      $codLocalEmisor,
      $codDoc,
      $igvbilling['document_number'],
      $igvbilling['Client_name'],
      $igvbilling['currency'],
      $igvbilling['Total_IGV'],
      $igvbilling['taxable_operations'] + $igvbilling['exempt_operations'] + $igvbilling['unaffected_operations'],
      $igvbilling['total_amount'],
      $sumDescTotal,
      $sumOtrosCargos,
      $sumTotalAnticipos,
      $igvbilling['total_amount'],
    ...
    $nameCAB = $companyInfo['ruc'] . "-" . $igvbilling['code'] . "-" . $igvbilling['series'] . "-" . $igvbilling['correlative'] . ".CAB";
    file_put_contents($path . $nameCAB, $cabecera_str);
  }

  private function createDET($igvbilling, $igvbilling_details, $companyInfo)
  {
    $detalles = [];

    foreach ($igvbilling_details as $detail) {
      $codigo = $detail['code'];
    ...
    $path = __DIR__ . "/../../files/DATA/";
    $nameDET =  $companyInfo['ruc'] . "-" . $igvbilling['code'] . "-" . $igvbilling['series'] . "-" . $igvbilling['correlative'] . ".DET";
    file_put_contents($path . $nameDET, $detalles_str);
  }

  private function createLEY($igvbilling, $companyInfo)
  {
    $leyendas = [];

    if ($igvbilling['total_amount'] >= 0.0) {
      $codLeyenda = "1000";
      $desLeyenda = $igvbilling['leyend'];
      $leyendas[] = $codLeyenda . "|" . $desLeyenda . "|";
    }

    if ($igvbilling['free_operations'] > 0.0 || $igvbilling['total_amount'] == 0.0) {
      $codLeyenda = "1002";
    ...
    $path = __DIR__ . "/../../files/DATA/";
    $nameLEY =  $companyInfo['ruc'] . "-" . $igvbilling['code'] . "-" . $igvbilling['series'] . "-" . $igvbilling['correlative'] . ".LEY";
    file_put_contents($path . $nameLEY, $leyendas_str);
  }

  private function createTRI($igvbilling, $companyInfo)
  {
    $tri = [];

    if ($igvbilling['taxable_operations'] > 0.0 && $igvbilling['Total_IGV'] > 0.0) {
      $codTributo = "1000";
      $nomTributo = "IGV";
      $CodTributo = "VAT";
      $baseImponible = floatval($igvbilling['taxable_operations']) + floatval($igvbilling['exempt_operations']) + floatval($igvbilling['unaffected_operations']);
      $tri[] = $codTributo . "|" . $nomTributo . "|" . $CodTributo . "|" . number_format($baseImponible, 2, '.', '') . "|" . number_format($igvbilling['Total_IGV'], 2, '.', '') . "|";
    }

    if ($igvbilling['free_operations'] > 0.0) {
      $codTributo = "9996";
      $nomTributo = "GRA";
      $CodTributo = "FRE";
      $mtoTributo = "0.00";
      $tri[] = $codTributo . "|" . $nomTributo . "|" . $CodTributo . "|" . $igvbilling['free_operations'] . "|" . $mtoTributo . "|";
    }

    if ($igvbilling['exempt_operations'] > 0.0) {
      $codTributo = "9997";
      $nomTributo = "EXO";
      $CodTributo = "VAT";
      $mtoTributo = "0.00";
      $tri[] = $codTributo . "|" . $nomTributo . "|" . $CodTributo . "|" . $igvbilling['exempt_operations'] . "|" . $mtoTributo . "|";
    }

    if ($igvbilling['unaffected_operations'] > 0.0) {
      $codTributo = "9998";
      $nomTributo = "INA";
      $CodTributo = "FRE";
      $mtoTributo = "0.00";
      $tri[] = $codTributo . "|" . $nomTributo . "|" . $CodTributo . "|" . $igvbilling['unaffected_operations'] . "|" . $mtoTributo . "|";
    }

    $tri_str = implode(PHP_EOL, $tri);
    $path = __DIR__ . "/../../files/DATA/";
    $nameTRI =  $companyInfo['ruc'] . "-" . $igvbilling['code'] . "-" . $igvbilling['series'] . "-" . $igvbilling['correlative'] . ".TRI";
    file_put_contents($path . $nameTRI, $tri_str);
  }

  private function createPAG($igvbilling, $companyInfo)
  {
    $pag = [];
    if ($igvbilling['payment_method'] == 1) {
      $mtoPago = "0.00";
      $pag[] = $igvbilling['description'] . "|" . $mtoPago . "|" . $igvbilling['currency'] . "|";
    } elseif ($igvbilling['payment_method'] == 2) {
    ...
    $path = __DIR__ . "/../../files/DATA/";
    $namePAG =  $companyInfo['ruc'] . "-" . $igvbilling['code'] . "-" . $igvbilling['series'] . "-" . $igvbilling['correlative'] . ".PAG";
