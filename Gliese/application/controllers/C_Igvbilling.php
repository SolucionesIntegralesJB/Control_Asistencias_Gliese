...
class C_Igvbilling extends Controller
{
    ...
    // --
    public function get_igvbilling()
    {
        $this->functions->validate_session($this->segment->get('isActive'));
        ...
        if ($request === 'GET') {
            ...
            if (!$campus_id) {
                ...
                $obj = $this->load_model('Igvbilling');
                $response = $obj->get_igvbilling($campus_id);
    ...
    // --
    public function get_igvbilling_by_id()
    {
        // --
        $this->functions->validate_session($this->segment->get('isActive'));
        // --
        ...
        if ($request === 'GET') {
            ...
            // --
            if (!empty($input['id_igvbilling'])) {
                // --
                $obj = $this->load_model('Igvbilling');
                // --
                $bind = array(
                    'id_igvbilling' => intval($input['id_igvbilling'])
                );
                // --
                $response = $obj->get_igvbilling_by_id($bind);
                // --
                switch ($response['status']) {
                        // --
                    case 'OK':
    ...
    public function get_igvbilling_Report()
    {
        try {
            $this->functions->validate_session($this->segment->get('isActive'));
            ...
            if (empty($input['id_igvbilling']) || empty($input['tipo'])) {
                throw new Exception('ID de igvbilling o tipo no proporcionado.');
            }

            $id_igvbilling = intval($input['id_igvbilling']);
            $tipo = intval($input['tipo']);

            $billingModel = $this->load_model('Igvbilling');

            $companyData = $billingModel->get_company();
            $reportData = $billingModel->get_igvbilling_Report($id_igvbilling);
            ...
            if ($reportData['status'] !== 'OK' || empty($reportData['result'])) {
                throw new Exception('No se encontró el reporte de igvbilling.');
            }
            ...
            switch ($tipo) {
                case 1:
                    include 'application/Reporte/Facturasinigv.php';
                    break;
                case 2:
                    include 'application/Reporte/Ticketsinigv.php';
                    break;
                default:
    ...
    public function Emitir_comprobante()
    {
        try {
            ...
            ob_start();
            include_once(__DIR__ . '/../efactura/template/facturasinigv.php');
            $output = ob_get_clean();
            ...
            if (isset($json_respuesta)) {
                ...
                if (json_last_error() === JSON_ERROR_NONE) {
                    ...
                    $billingModel->get_igvbilling_status($updateData);
    ...
    public function send_email()
    {
        try {
            ...
            if (empty($input['id_igvbilling']) || empty($input['email'])) {
    ...
    public function xml_igvbilling()
    {
        try {
            $this->functions->validate_session($this->segment->get('isActive'));
            ...
            $input = filter_input_array(INPUT_GET);
            if (empty($input['id_igvbilling'])) {
                throw new Exception('ID de igvbilling no proporcionado.');
            }
